...
- **The unchecked-catalog seam is for repair only** (`catalog_unchecked`, `load_container_unchecked`, `save_container_unchecked` — ADR-045). Ordinary operations use `store.catalog()` and get [R24] fatality; the unchecked twins exist so an operation that can only *reduce* incoherence still works on a repository nothing else can load. Today that is `container_service::remove_member` / `remove_root` (plus `repo validate`'s pre-existing exemption), and the read-only `load_container_for_repair` callers `manifest_service::unset_manifest_root_container` and `doctor_service::instance_id_referenced` (see ADR-045's Consequences). Do not reach for them to make an ordinary command "more forgiving" — a caller that can introduce a dangling reference must see the checked catalog.
...
- **Merge rights, revival, baseline-honesty:** same rules as `srs/CLAUDE.md`'s "Gates and choreography" section — consume, don't clone. In short: a PR states its decision mode and door, and an agent may merge only when the mode is clear/complicated AND the change is Door 1 or non-normative (`gate:auto-merge`); complex, Door 2 or Door 3 carries `gate:owner-merge` and the agent never merges; a branch predating the newest `rfc-decision-*` record is re-derived against the decision log before shipping; a "pre-existing failure" claim is proven against the last GREEN master CI run, never current master.
