...
- `resolve_relations_source` returning raw text (vs. a parsed collection) is required so `validate` keeps emitting a JSON-parse diagnostic on a malformed relations file rather than hard-erroring.
