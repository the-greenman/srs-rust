...
const STRATEGY_MISSION_FIELDS = [
  "status", "ratifiedOn", "decisionAuthority", "purpose", "muDemocracy", "srs",
  "firstApplication", "productPromise", "humanAiConstitution", "blueprint",
  "protocol", "view", "portableDocumentPromise", "storageIndependence",
  "scopeBoundary", "stabilityPromise", "semanticSourcePlan", "roadmapTests",
];
...
function validateStrategy(model) {
  ...
  if (!model.mission || typeof model.mission !== "object") {
    errors.push("missing mission");
  } else {
    for (const field of STRATEGY_MISSION_FIELDS) {
      const value = model.mission[field];
      if (value == null || value === "" || (Array.isArray(value) && !value.length)) errors.push(`mission: missing ${field}`);
    }
    if (model.mission.status !== "ratified") errors.push("mission: status must be ratified");
  }
...
function renderStrategy(model) {
  ...
  const lines = [
    ...
    `**Status:** ${mission.status[0].toUpperCase()}${mission.status.slice(1)} on ${mission.ratifiedOn}. ${mission.decisionAuthority}`, "",
    `> ${mission.purpose}`, "",
    "### μDemocracy", "",
    mission.muDemocracy, "",
    "### SRS", "",
    ...
    mission.firstApplication, "",
    mission.productPromise, "",
    "### Human–AI constitution", "",
    `> ${mission.humanAiConstitution}`, "",
    `- **Blueprint:** ${mission.blueprint}`, "",
    `- **Protocol:** ${mission.protocol}`, "",
    `- **View:** ${mission.view}`, "",
    "### Sovereignty promises", "",
    `- **Portable document:** ${mission.portableDocumentPromise}`, "",
    `- **Storage independence:** ${mission.storageIndependence}`, "",
    "### Scope boundary", "",
    mission.scopeBoundary, "",
    "### Stability promise", "",
    mission.stabilityPromise, "",
    "### Semantic source transition", "",
    mission.semanticSourcePlan, "",
    "### Roadmap test", "",
    "Roadmap boundaries and critical-path work are judged against these questions:", "",
    ...mission.roadmapTests.flatMap((test, index) => [`${index + 1}. ${test}`, ""]),
    "## Capability map", "",
