...
**`validate_repository` type-specific cross-record constraints (`validation.rs`, #555):** Two checks in `validate_repository` dispatch on concrete type identity rather than a generic extension point: (a) `com.semanticops.core/purpose` identity-type validation (RFC-018 I-81), and (b) `com.semanticops.spec/invariant` number uniqueness (#555). Both are intentional — the "Negative / trade-offs" section above acknowledges that "cross-record constraints cannot be expressed through the generic path alone." A follow-up design track should explore a `uniqueFields: [fieldId]` annotation on RecordType definitions so that per-type uniqueness rules can be declared in package data rather than library code.
