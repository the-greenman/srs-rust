...
- The same pattern was subsequently extended to `create_note_in_context` (issue #455) using the inline form `let _ = delete_note(store, &id)` rather than a named helper (one call site, no clarity gain). The applicable caveats from the Negative section differ slightly from the record case: the file-before-index ordering risk (first bullet) and the `let _ = …` error-swallowing (third bullet) both carry over to `delete_note`. The `find_relations_referencing_instance` pre-mutation call (second bullet) does **not** apply — `delete_note` has no such call and goes straight to `load_manifest` → `delete_instance_file` → `write_manifest`; a failure in `delete_note` during rollback therefore touches nothing, leaving the note intact in the manifest rather than producing a dangling entry.
