...
**CLI surface.** `mcp serve` (stdio MCP: `initialize`, `resources/list`, `resources/read`, `tools/list`, `tools/call` for `repo_validate` / `find` / `type_schema` / `record_create` / `relation_create` / `note_create`).
...
| `mcp serve` (MCP stdio server: resources map/navigation/record/container/view/**type** + tools `repo_validate`/`find`/`type_schema`/`record_create`/`relation_create`/`note_create` + **prompts** `prompts/list`/`prompts/get`, ADR-037 + #692 amendment + #682 prompts) | S42 (incl. the #692 discover-then-author step and #682 prompts step 10b); 15 crate tests in `crates/srs-mcp/` (unit + duplex-transport integration incl. e2e discovery loop) + 2 binary-level handshake tests in `crates/srs-cli/tests/mcp_serve.rs` + 4 unit tests in `crates/srs-mcp/src/prompts.rs` |
