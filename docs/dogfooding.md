...
| `mcp serve` (MCP stdio server: resources map/navigation/record/container/view/**type** + tools `repo_validate`/`find`/`type_schema`/`record_create`/`relation_create`/`note_create`, ADR-037 + #692 amendment) | S42 (incl. the #692 discover-then-author step); 15 crate tests in `crates/srs-mcp/` (unit + duplex-transport integration incl. e2e discovery loop) + 2 binary-level handshake tests in `crates/srs-cli/tests/mcp_serve.rs` |
