...
5. Delete a relation and confirm it disappears from `relation list` without touching either endpoint. In a repo with a declared `relationsPath`, confirm the `path` field in the `relation delete` response matches the declared path — not the hardcoded default `"relations/relations-collection.json"` (regression guard for #571).
...
**Negative case (validate reads the authoritative file — #548).** After a successful `relation create` (which writes to the manifest's declared `relationsPath` when set, otherwise `relations/relations-collection.json`, **not** `relations/relations.json` — see #560), edit that file to reference a relation type not installed in the package, then run `srs repo validate --repo <repo> --pretty`. Confirm `ok: false` with an `E1` diagnostic whose path is the authoritative file. (Before #548, validate read only `relations/relations.json`, so a repo whose relations live at the default `relations-collection.json` validated green with the bad edge silently unread.)

**Done when.** Relations appear/disappear in `relation list`; `record successor` produces both a successor record and the supersession edge; neither endpoint's lifecycle state changed as a side effect of any relation operation; `record successor` with an unknown `relationType` fails before writing any record; an E4-violating `relation create` is rejected at write time and leaves no relation file behind while an E4-satisfying one succeeds; `repo validate` surfaces a bad edge living in the authoritative relations file (manifest `relationsPath` if declared, otherwise `relations/relations-collection.json`), not only in `relations/relations.json`; and in a repo with a declared `relationsPath` and no pre-existing relations file, the **first** `relation create` writes to the declared path — not to the hardcoded default (regression guard for #560); and `relation delete` on a repo with a declared `relationsPath` returns `path` equal to the declared path, not the hardcoded default `"relations/relations-collection.json"` (regression guard for #571).
...
**CLI surface.** `vocabulary create`, `vocabulary get`, `vocabulary list`, `vocabulary term-create`, `vocabulary derive-tag-set`, `vocabulary promote`, `term list`, `term get`, `lifecycle list`, `lifecycle get`, `record tag`, `record transition`, `field create`, `type create`, `type schema`.
...
3a. **`vocabularyRef`-backed select field authoring contract (srs-rust#1002).** Create a `closed`-mode vocabulary with two `active` Terms and one `deprecated` Term. Create a Field with `fieldType: {datatype: "string", valueDomain: "closed", vocabularyRef: "<vocabId>"}` and assign it on a Type. Run `srs type schema <typeId>` and confirm the field's property carries `enum` = the two active term keys only (the deprecated one excluded), plus `x-srs-vocabulary-id` and `x-srs-vocabulary-terms` (each entry's `key`/`label`/`description`). This is what an agent reads before `record create` to know which values are valid — before #1002 the property had no `enum` at all for a `vocabularyRef` field, only for literal `allowedValues`.
...
This is the issue-#254/#256 capability: a single `resolve_container_view` projection (service → CLI payload → WASM binding, per `docs/architecture/capability-layering.md`) returning the container root record, the ordered member records (Tier-0, Tier-1, or Tier-2; full `Record` present only for Tier-2), and the **column/field spec** resolved from a DocumentView section's `renderViewId → View.field_views`. Column-source precedence is [ADR-018](adr/018-container-view-column-source-precedence.md): the section targeting this container wins, else the first section by `order` with a `renderViewId`, else empty columns. Tier-0/1 members carry `record: null` and a `displayLabel` from the manifest index entry's `title` (falling back to `instanceId`); Tier-2 members carry a full `record` and a core-resolved `displayLabel`.

**Capabilities exercised.** Container membership (roots-first, deduped); DocumentView → View `field_views` column projection (visible-false exclusion, `order` sort, `displayLabel` override → field `name` fallback); core `record_display_label` reuse for Tier-2 member/root labels; Tier-0/1 member projection (manifest index title → `displayLabel`, `record: null`, `isVisibleByDefault: true`, #256); unknown-tier members skipped with a diagnostic; `--view-id` override vs. root-type matching; non-fatal diagnostics vs. hard errors; `ColumnSpec.isIdentityColumn` (RFC-020, ADR-023, #376) — the resolved column matching the single-Type-anchored DocumentView's effective `identityFieldId`. The anchor repo is the `rfc008-container-subset` fixture (heterogeneous container `…3500`, text-view `…3504`).
...
8. **Tier-0/1 member projection (#256)** — in a fresh scratch repo, create a Field, a Type `decision`, and a Tier-2 Record (the container root). Create a Tier-0 Note with `{"title":"Background Note","sections":[{"name":"context","content":"..."}]}`. Create a container with the record as root, then add the note as a member. Run `srs container resolve-view <containerId> --repo <repo>`. Confirm the payload contains two members: the Tier-2 record entry carries `"tier":2`, a full `record` object, and its `displayLabel`; the Tier-0 note entry carries `"tier":0`, `"isVisibleByDefault":true`, `"displayLabel":"Background Note"` (from the manifest index title), and **no `record` field** (it is omitted by `skip_serializing_if`). Run `srs repo validate` → `ok: true`, 0 errors.
...
**Verified 2026-07-16 (#256):** step 8 end-to-end on branch `feat/256-tier01-members-container-view`. Repo `/tmp/dogfood-tier01-256` — Field `title` (text), Type `com.example.dogfood/decision` with `identityFieldId`, Tier-2 record "The Decision" as container root, Tier-0 Note "Background Note" as member. `container resolve-view` returns two members: `{"tier":2,"displayLabel":"The Decision","record":{…},"isVisibleByDefault":true}` and `{"tier":0,"displayLabel":"Background Note","isVisibleByDefault":true}` — no `record` key on the Tier-0 entry. `repo validate` → `ok: true`, 0 errors.
...
**Steps.**
1. `srs-gov repo-create --output /tmp/dogfood-srs-gov-list.srsj --title "Acme Co-op"` → a fresh governance `.srsj`. Pass `--namespace com.example.acme-co-op` to override the derived namespace (`com.example.<slug>` from title); the explicit namespace appears in the success output under `Namespace:` and in `manifest.namespace`. Confirm the stamped seed's decision-log DocumentView is a `type-query` (regenerated asset): `srs container resolve-view <decisionLogId> --repo <repo>` → `payload.containerView.excludeLifecycleStates: ["superseded","closed"]`.
...
**Verified 2026-07-22 (#654) — `.srsj` source determinism.** Packing from a `.srsj` (JsonStore) source twice produces byte-identical archives; `manifest.json` keys are emitted in sorted order from both JsonStore and MemoryStore sources. Live run: `srs archive pack --repo /tmp/dogfood-654.srsj --store json --output run1.srs` and run2.srs produced identical 3280-byte files; both archives' `manifest.json` had keys sorted lexicographically (`$schema` → `container` → `containerIndex` → `createdAt` → … → `title`).
...
**CLI surface.** `mcp serve` (stdio MCP: `initialize`, `resources/list`, `resources/read`, `tools/list`, `tools/call` for all 13 tools: `repo_validate` / `find` / `type_schema` / `record_create` / `relation_create` / `note_create` / `record_update` / `record_transition` / `record_allowed_transitions` / `record_successor` / `note_graduate` / `container_member_add` / `container_member_remove`).
...
9. **Type discovery (#692):** `resources/list` includes `com.example.dogfood/decision` at `srs://<repoId>/type/<typeId>`; `tools/call type_schema` `{typeId}` returns the authoring schema — each property carries `x-srs-field-id`, `required` lists `title`; a further `record_create` built **from those discovered fieldIds** (not the setup's variables) succeeds and `repo_validate` stays clean.
10. Close stdin → process exits `0`; stdout carried only JSON-RPC throughout.

10b. **Prompts surface (#682):** first install a blueprint (any valid JSON, `rootTypes` may reference `com.semanticops.core/purpose`):

```bash
echo '{
  "id":"","namespace":"com.example.dogfood","name":"decision-record","version":1,
  "description":"A decision authoring guide",
  "rootTypes":[{"typeId":"3c000001-0000-4000-a000-000000000001","typeVersion":1}],
  "structure":[],"requiredTypes":[],"aiGuidance":"Capture context, options, and rationale.",
  "createdAt":"2026-07-24T00:00:00Z"
}' | $SRS blueprint create --repo $REPO
```

Then in an MCP session: `prompts/list` → returns one entry; `name` is the blueprint UUID; `description` is `"com.example.dogfood/decision-record v1: A decision authoring guide"` (human-readable in pickers). `prompts/get` with that UUID → one `Role::User` message whose `text` contains the rendered brief (blueprint heading + `aiGuidance` + type field table). `prompts/get` with an unknown UUID → JSON-RPC error `{"code": -32602, "message": "prompt not found: ..."}`.
...
**Verified 2026-07-22 (#692 extension).** Discover-then-author loop on the same scratch repo: type enumerated in `resources/list` ✓, `type_schema` returned `x-srs-field-id` for both `title` and `decision_statement` (matching the setup's UUIDs) with `required: ["title"]` ✓, record created from the discovered ids ✓, unknown-typeId rejected (`type not found: …`) ✓, `repo_validate` 0 diagnostics ✓, clean exit ✓.

10c. **Second-wave write tools (#680):** using the same repo (with a `com.example.dogfood/proposal` lifecycle type, `supersedes` + `refines` relation types, and a container already installed):

- `tools/call record_update` `{instanceId: v1_id, fieldValues: [{fieldId, value: "Meet weekly — updated"}]}` → `isError: false`, returned record carries the new value.
- `tools/call record_allowed_transitions` `{instanceId: proposal_id}` (proposal is in `draft`) → `currentState: "draft"`, `transitions: [{name: "promote", to: "active", toIsFinal: false}]`, `isImmutable: false`.
- `tools/call record_transition` `{instanceId: proposal_id, byTransition: "promote"}` → `record.lifecycleState: "active"`, `warnings: []`.
- `tools/call record_successor` `{predecessorId: v2_id, relationType: "supersedes", fieldValues: [{fieldId, value: "Meet monthly"}]}` → `isError: false`, returns `{record: {instanceId: v3_id, …}, relation: {relationType: "supersedes", sourceInstanceId: v3_id, targetInstanceId: v2_id}}`.
- `tools/call note_graduate` `{noteId, type: "com.example.dogfood/decision", fieldValues: […]}` → `note.graduatedAt` non-null, `record.instanceId` non-empty.
- `tools/call container_member_add` `{containerId, instanceId: record_id}` → `memberInstanceIds` now includes `record_id`. Idempotent: second call returns same list unchanged.
- `tools/call container_member_remove` `{containerId, instanceId: record_id}` → `memberInstanceIds` no longer includes `record_id`.
- `tools/call repo_validate` → `diagnostics: []`.

**Verified 2026-07-24 (#680).** All seven second-wave tools verified against branch binary at `/tmp/dogfood-s42-680`: `record_update` field-count 2 ✓; `record allowed-transitions` state=draft, 1 transition, isImmutable=False ✓; `record transition promote` state=active ✓; `record successor` supersedes relation, correct endpoints ✓; `note graduate` graduatedAt set, record instanceId present ✓; `container members add` idempotent ✓; `container members remove` leaves list at prior count ✓; `repo validate` 0 errors ✓.
...
## S45 — Export a container as an Open Knowledge Format bundle (`render okf-bundle`, #677)
...
```bash
SRS=<path-to-srs-bin>
REPO=/tmp/dogfood-okf
rm -rf "$REPO" /tmp/dogfood-okf-export
...
TYPE_ID=$(echo "$TYPE_JSON" | $SRS type create --repo "$REPO" | python3 -c "import sys,json; print(json.load(sys.stdin)['payload']['type']['id'])")
...
**Negative case.** Run with a `--container` ID that does not exist in the repo:

```bash
$SRS render okf-bundle \
  --repo "$REPO" \
  --container 00000000-0000-0000-0000-000000000000 \
  --output /tmp/dogfood-okf-neg \
  --pretty
# → ok: false; diagnostics[0]: "container not found: 00000000-0000-0000-0000-000000000000"
# No files written to /tmp/dogfood-okf-neg
```
...
### S25 — Add something to a container, and get back out when a repository is already broken (#841, ADR-045)

**Intention.** *"I want to put this record in a container. If I fumble the id, I want to be told — not to discover an hour later that the whole repository has stopped loading. And if a repository reaches me already in that state, I want to fix it with the CLI, not a text editor."*

This is the trust-boundary half of container membership. A `rootInstanceIds` / `memberInstanceIds` entry that resolves to nothing is a fatal `SRS038-R13-DANGLING-REFERENCE`, and under [R24] a fatal diagnostic fails the *whole* catalog load — so a bad membership write does not break one container, it makes every command on the repository fail. Before #841 the add commands accepted such an id, reported `ok: true`, and persisted it, and the `remove` that would undo it failed on the same fatal build. The scenario covers both halves: the guard, and the way back out.

**Capabilities exercised.** Membership writes validated at the service boundary (ADR-010) so the CLI, MCP and WASM adapters inherit the same rule; `repo validate`'s [R24] reporting exemption as the *discovery* half of a repair; membership **removal** as the *repair* half (ADR-045 — removal can only reduce the reference set, so it reads and writes through the unchecked catalog).

**CLI surface.** `note create`, `container members add`, `container roots add`, `container roots remove`, `container create`, `repo validate`, `repo navigation`.
...
SRS=./target/debug/srs
R=/tmp/dogfood-841
$SRS repo create --repo "$R" --namespace com.example.dogfood --package-name dogfood \
    --package-version 1.0.0 --srs-version 2.0-draft
ROOT=$(python3 -c "import json;print(json.load(open('$R/manifest.json'))['container']['containerId'])")

# 1. Happy path — the instance exists first, then joins the container.
NOTE=$(echo '{"title":"Charter","sections":[{"name":"why","content":"To prove membership writes resolve."}]}' \
  | $SRS note create --repo "$R" | python3 -c "import json,sys;print(json.load(sys.stdin)['payload']['note']['instanceId'])")
$SRS container members add --repo "$R" "$ROOT" "$NOTE"     # ok: true, member listed
$SRS container members add --repo "$R" "$ROOT" "$NOTE"     # idempotent — same list back
```

**Negative case.** All three unresolvable forms are rejected, and the repository is untouched:

```bash
$SRS container roots add --repo "$R" "$ROOT" ""
# → {"ok":false,"diagnostics":["invalid input: instance_id must not be empty"]}
$SRS container roots add --repo "$R" "$ROOT" "   "
# → same — whitespace is not an id
$SRS container members add --repo "$R" "$ROOT" dddddddd-dddd-4ddd-8ddd-dddddddddddd
# → {"ok":false,"diagnostics":["instance not found: dddddddd-dddd-4ddd-8ddd-dddddddddddd"]}
$SRS repo validate --repo "$R"      # ok: true, 0 errors — nothing was written
```

**Recovery round trip.** `container create` still takes the membership list wholesale (see srs-rust#845), which is both the remaining way in and a convenient way to manufacture the broken state:

```bash
GHOST=dddddddd-dddd-4ddd-8ddd-dddddddddddd
CID=22222222-3333-4444-5555-666666666666
echo "{\"containerId\":\"$CID\",\"title\":\"Bricked\",\"rootInstanceIds\":[\"$GHOST\"]}" \
  | $SRS container create --repo "$R"                  # ok: true — the write that breaks it

$SRS repo navigation --repo "$R"                       # ok: false — catalog load failed
$SRS container roots list --repo "$R" "$CID"           # ok: false — same
$SRS note list --repo "$R"                             # ok: false — same

$SRS repo validate --repo "$R"                         # STILL WORKS — names the container file and the id
$SRS container roots remove --repo "$R" "$CID" "$GHOST"  # STILL WORKS — ok: true, rootInstanceIds: []
$SRS repo validate --repo "$R"                         # ok: true — repository loads again
```

**Done when.** The happy-path add succeeds and is idempotent; all three unresolvable forms return `ok: false` with a message that names the actual problem (empty argument vs unresolvable id — not one message for both) and leave the container byte-identical; on a bricked repository `repo validate` and `container roots/members remove` both still return a useful answer while every ordinary command fails; and after the removal the repository loads again with `repo validate` reporting 0 errors. Nothing in the round trip touches a JSON file by hand.

**Verified 2026-08-14 (#841)** on branch `feat/841-container-membership-blank-id`, repo `/tmp/dogfood-841`: all three negative forms rejected; happy path and idempotent re-add `ok: true`; brick → `repo navigation` / `container roots list` / `note list` all `ok: false` with `SRS038-R13-DANGLING-REFERENCE`, `repo validate` reporting `[containers/bricked-22222222.json]` and the ghost id; `container roots remove` `ok: true` with `rootInstanceIds: []`; `repo validate` `ok: true` afterwards.

> **Known unrelated failure in this scenario.** After the repair, `repo navigation` reports `failed to load record at "records/notes/charter-….json": missing field 'typeId'` — a Tier-0 note that is an ordinary member of the root container is read by the record-only loader. This is **srs-rust#842**, independent of #841: it reproduces on a clean repo with no bricking involved (`repo create` → `note create` → `container members add` → `repo navigation`). Use `repo validate` as this scenario's load check until #842 lands.


### S47 — Declare a Container's typing anchor explicitly (`anchorInstanceId`, srs#446/I-145)

**Intention.** *"My container has multiple root candidates and I want to say, declaratively, which record's Type governs which DocumentView applies — not have that governed by array position, which can silently move when a member list is edited."*

**Capabilities exercised.** `Container.anchorInstanceId` (RFC-009 Change A, amended by srs#446): `container_service::typing_anchor_instance_id` prefers the declared anchor over the transitional `rootInstanceIds[0]` fallback; `document_views_for_container` resolves DocumentView matching against it; `repo validate`'s I-145 checks (Error when the anchor is not a member of the container, Warning naming the Continuity-flip expiry when the transitional fallback is in effect); `container resolve-view`'s inline ADR-044-consistent transitional-fallback diagnostic.

**CLI surface.** `repo create`, `note create`, `container update`, `repo validate`.

**Steps.**

```bash
SRS_BIN=target/debug/srs
SCRATCH=/tmp/dogfood-anchor-s47
rm -rf "$SCRATCH"

$SRS_BIN repo create --repo "$SCRATCH" --namespace com.example.s47 --pretty
ROOT_ID=$(python3 -c "import json; print(json.load(open('$SCRATCH/manifest.json'))['container']['containerId'])")

ROOT_NOTE=$($SRS_BIN note create --repo "$SCRATCH" <<'EOF' | python3 -c "import sys,json; print(json.load(sys.stdin)['payload']['note']['instanceId'])"
{"title": "Root Section", "sections": []}
EOF
)

# 1. No anchorInstanceId yet (transitional fallback in effect once a root exists).
echo "{\"rootInstanceIds\":[\"$ROOT_NOTE\"]}" | $SRS_BIN container update --repo "$SCRATCH" "$ROOT_ID" --pretty
$SRS_BIN repo validate --repo "$SCRATCH" --pretty

# 2. Negative case: point anchorInstanceId at a record that is NOT a member.
STRAY=$($SRS_BIN note create --repo "$SCRATCH" <<'EOF' | python3 -c "import sys,json; print(json.load(sys.stdin)['payload']['note']['instanceId'])"
{"title": "Stray", "sections": []}
EOF
)
echo "{\"anchorInstanceId\":\"$STRAY\"}" | $SRS_BIN container update --repo "$SCRATCH" "$ROOT_ID" --pretty
$SRS_BIN repo validate --repo "$SCRATCH" --pretty

# 3. Fix: declare the anchor as the actual root member.
echo "{\"anchorInstanceId\":\"$ROOT_NOTE\"}" | $SRS_BIN container update --repo "$SCRATCH" "$ROOT_ID" --pretty
$SRS_BIN repo validate --repo "$SCRATCH" --pretty
```

**Done when.** Step 1's `repo validate` returns `ok: true` with a `"warning"` diagnostic whose message contains `"I-145"` and `"Continuity flip"`. Step 2's `repo validate` returns `ok: false` with a diagnostic containing `"I-145"` and `"is not in rootInstanceIds or memberInstanceIds"`. Step 3's `repo validate` returns `ok: true` with **no** `I-145` diagnostic at all.

**Verified 2026-09-01** on branch `feat/891-container-anchor-instance-id`, repo `/tmp/dogfood-anchor`: all three steps produced exactly the diagnostics described above via the real CLI binary (not just unit tests).
...
| `type` (create/get/list/schema/update/delete) | S2; `type schema` `enum` for a `vocabularyRef`-backed select field (active terms only, `x-srs-vocabulary-terms`, srs-rust#1002) — S6 step 3a; **`type create` rejects a `FieldAssignment.fieldId` that would fail RFC-038 [R13] at catalog load (srs-rust#1039)** — validates against the same on-disk definition set [R13] itself resolves against (excluding the ADR-025 implicit-core merge), before writing anything. Dogfooded on branch: `type create` with a fieldId referencing the implicit core `statement` field → `ok: false`, `"field not found: <id>"`, `repo validate` stays 0 errors (nothing written), and `record create --type com.semanticops.core/purpose` keeps working (no cascade); `type create` with a legitimate local field id → `ok: true`, `repo validate` 0 errors |
...
| `record get-field-value` (lookup by package-defined field name) | CLI: _gap — no command yet, deferred to #537_; WASM binding (`SrsRepository.get_field_value_by_name`) verified via unit tests in `crates/srs-repository/src/record_store.rs` (5 tests including cross-store roundtrip) and import-reachability tests in `crates/srs-bindings/src/lib.rs` (#536) |
...
| `container update` (`rootInstanceIds`/`memberInstanceIds`/`identityInstanceId` patch fields, `deny_unknown_fields`) | S4 (extended — patch membership fields, unknown-field error); verified end-to-end in worktree dogfood (#422); `anchorInstanceId` patch field (srs#446, RFC-009 Change A) — S47 |
| `container resolve-view` (structured container view, `--view-id`; `ColumnSpec.isIdentityColumn`, ADR-023, #376; Tier-0/1 members projected, #256) | S14 |
