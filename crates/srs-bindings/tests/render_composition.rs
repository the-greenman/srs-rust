//! Integration test for the WASM `render_composition` binding.
//!
...
//! test the underlying service directly via `srs_repository::srsj::open_srsj`, since `to_js()` calls
...
#[test]
fn render_composition_json_format_returns_projection() {
    ...
    assert_eq!(
        proj.composition_id, "5a3ce87e-8340-4d91-a140-ab56b57f704f",
...
#[test]
fn render_composition_markdown_format_no_projection() {
    let store = gallery_store();
    let result = render_composition(RenderCompositionOptions {
        store: &store,
...
#[test]
fn render_composition_unknown_view_errors() {
    let store = gallery_store();
    let result = render_composition(RenderCompositionOptions {
        store: &store,
