    ...
    serde_json::json!({
        ...
        "manifest": {
            ...
            "instanceIndex": [
                {"instanceId": ROOT_ID, "path": format!("records/tier-2/{ROOT_ID}.json"), "tier": 2},
                {"instanceId": MEMBER_ID, "path": format!("records/tier-2/{MEMBER_ID}.json"), "tier": 2},
                {"instanceId": SUPERSEDED_ID, "path": format!("records/tier-2/{SUPERSEDED_ID}.json"), "tier": 2}
            ],
            "packageRef": {"mode": "local", "path": "package"}
        },
        "data": {
...
fn resolve_container_view_is_visible_by_default_false_serialised() {
    ...
    // Locate the superseded member by instanceId in the serialised output.
    let superseded = members
        .iter()
        .find(|m| m["instanceId"] == SUPERSEDED_ID)
        .expect("superseded member in JSON");
    assert_eq!(
        superseded["isVisibleByDefault"],
        serde_json::Value::Bool(false),
        "superseded member must have isVisibleByDefault: false in serialised JSON"
    );

    // Sanity: root and active member are visible.
    let active = members
        .iter()
        .find(|m| m["instanceId"] == MEMBER_ID)
        .expect("active member in JSON");
    assert_eq!(active["isVisibleByDefault"], serde_json::Value::Bool(true));
}
