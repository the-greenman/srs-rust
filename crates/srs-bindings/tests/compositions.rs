//! Integration test for the WASM `compositions_for_container` binding.
//!
...
//! export compiles.
//!
//! The service joins a container's root-instance Type to a Composition's `rootTypeRefs`
...
/// Minimal `.srsj`: a container whose root record binds `TYPE_ID` v1, and a Composition whose
    ...
    serde_json::json!({
        ...
        "data": {
            "package/package.json": {
                ...
                "relationTypes": [],
                "views": [],
                "compositions": ["compositions/bound.json"],
                "blueprints": []
            },
            "package/compositions/bound.json": {
...
#[test]
fn compositions_for_container_returns_matching() {
    let store =
        srs_repository::srsj::open_srsj(&bound_view_srsj()).expect("bound-view srsj must load");
    let views = view_service::compositions_for_container(&store, CONTAINER_ID)
...
fn compositions_for_container_empty_when_unbound() {
    let store = gallery_store();
    let views =
        view_service::compositions_for_container(&store, "138e2fac-6a8a-4a06-9511-5aefd99ceae9")
            .expect("lookup must succeed");
    assert!(
        views.is_empty(),
